    ...
    def validate(self, attribute):
        if (self._minLength != None and len(attribute) < self._minLength) or (self._maxLength != None and len(attribute) > self._maxLength):
            if self._minLength == None and self._minLength != None:
                raise ValidationError(unicode(self._label.get(self._messageMax)) % (self._messageParameter, self._maxLength))
            if self._minLength != None and self._minLength == None:
                raise ValidationError(unicode(self._label.get(self._messageMin)) % (self._messageParameter, self._minLength))
            if self._minLength != None and self._minLength != None:
                raise ValidationError(unicode(self._label.get(self._messageMinMax)) % (self._messageParameter, self._minLength, self._maxLength))
