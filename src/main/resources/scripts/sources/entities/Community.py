...
class CommunityManager(Container):
    ...
    def generateZpkNumber(self, community):
        ...
        zpkWaitingForAccountRF = manager.generateZpkForCommunity(community, "WAITING_FOR_ACCOUNT_RF")
        community.getZpks().add(zpkWaitingForAccountRF)
    ...
    def addContractors(self, community):
        ...
        for i in range(int(self._svars.get(self._prefix + 'contractorsCount'))): 
            self._svars.put(str(i) + '_countractor_communityId', str(community.getId()))
            contractorsManager.setPrefix(str(i) + '_countractor_')
