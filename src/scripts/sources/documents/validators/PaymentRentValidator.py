...
from structures.validators.common.NotEmptyValidator import NotEmptyValidator
...
class PaymentRentValidator(Validator):
    ...
    def validatePosition(self, position):
        ...
        self.check(position.getDescription(), [NotEmptyValidator(messageParameter = self._label.get('field.description'))])
        self.check(position.getClientName(), [NotEmptyValidator(messageParameter = self._label.get('field.clientName'))])
